...
const Analyze = () => {
  ...
  useEffect(() => {
    ...
    const top = results
      .slice(0, 2)
      .map((crop) => `${crop.name || crop.id} (${Math.round(Number(crop.confidence || 0))}%)`)
      .join(', ')

    const summary = i18n.language === 'ta'
      ? isLoading
        ? 'பயிர் பரிந்துரை பகுப்பாய்வு நடைபெறுகிறது.'
        : results.length > 0
          ? `பயிர் பரிந்துரை முடிவு தயார். சிறந்த பரிந்துரைகள்: ${top}.`
          : 'பயிர் விவரங்களை அளித்து பரிந்துரை பெறலாம்.'
      : isLoading
        ? 'Crop recommendation analysis is running.'
        : results.length > 0
          ? `Crop recommendation results are ready. Top suggestions: ${top}.`
          : 'Provide field details to generate recommendations.'
  ...
  }, [i18n.language, isLoading, results])
